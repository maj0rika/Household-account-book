...
// 구현체를 환경변수로 전환
// 개발/테스트: OpenAI GPT
// 프로덕션: KIMI 2.5 (비용 최적화)
```

| 단계        | LLM                | 이유                                     |
| ----------- | ------------------ | ---------------------------------------- |
| 개발/테스트 | OpenAI GPT-4o-mini | 빠른 프로토타이핑, 한국어 파싱 품질 검증 |
| 프로덕션    | KIMI 2.5           | 비용 절감, 배포용                        |
...
| **LLM**         | OpenAI SDK (교체 가능)  | 개발: GPT-4o-mini / 프로덕션: KIMI 2.5                         |
...
├── tsconfig.json
├── package.json
└── .env.local                  # OPENAI_API_KEY, KIMI_API_KEY 등
```
...
OPENAI_API_KEY        (개발/테스트용)
LLM_PROVIDER          ("openai" | "kimi")
KIMI_API_KEY           (프로덕션용)
