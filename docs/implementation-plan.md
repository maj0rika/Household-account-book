...
> **단일 기준 문서**: 현재 구현 Phase, 확장 Phase, backlog, 운영 메모, 결정 로그는 이 문서 한 곳에서 관리한다.
...
## 통합 backlog / 운영 메모 / 결정 로그

이 문서는 현재 저장소에서 사용하는 **유일한 계획 문서**다.

### 문서 운영 원칙

- 새 계획, backlog, 운영 메모, 결정 로그는 모두 이 문서에 기록한다;
- `docs/roadmap-v2-todo-phase-plan.md`는 과거 링크 호환을 위한 레거시 포인터로만 유지한다;
- 기능 단위 backlog는 `TX`, `PF`, `AI`, `DM`, `AN`, `OPS`, `SEC` 식별자를 유지한다;
- 장기 큰 단위는 `Phase`로 유지하고, backlog 식별자는 그 Phase 안의 실행 아이템으로 해석한다;
...
- [x] **TX-08** 거래/자산 저장 성공 시 해당 화면으로 명확히 리다이렉트/포커스
- [x] **TX-09** 월 전환 상태와 주간지출(focusDate) 필터 범위 정합성 개선 (과거월에서 최근 7일 필터가 0건처럼 보이는 리스크 해소)
- [ ] **TX-10** 입력창 `X` 버튼으로 초안/결과 일괄 삭제
  - 현재 확인: `NaturalInputBar`에는 첨부 이미지 제거 `X`만 있고 텍스트 전체 초기화 액션은 없다;
  - 범위 후보: 입력창 텍스트 비우기, 파싱 결과 시트 열림 상태면 결과도 함께 닫기 또는 전체 삭제;
- [ ] **TX-11** 고정거래 관리 UI에 수정 진입점 추가
  - `RecurringTransactionManager` 목록 행에서 삭제만 가능하므로 수정 플로우가 필요하다;

#### P1 (성능/로딩 구조)

- [x] **PF-01** 페이지 전환 성능 개선: 라우팅 프리패치(탭/사이드바 Link + RoutePrefetcher) 적용
- [x] **PF-02** Above-the-fold 우선 로딩: 상단 카드/캘린더 우선, 하단 분석 영역 지연 렌더(content-visibility) 적용
- [x] **PF-03** 차트/무거운 컴포넌트 dynamic import + skeleton
- [x] **PF-04** 월 전환 시 비관적 전체 블락 대신 부분 로딩(섹션별 pending) 고도화 2차
- [x] **PF-05** 사용자가 "멈춤"으로 느끼지 않게 상태 문구/마이크로 애니메이션 통일(월 데이터 로딩 문구)
- [ ] **PF-06** `zustand` / `react-query` 도입 검토
  - 현재 스택에는 두 라이브러리가 없으므로, 실제 병목 구간과 캐시 무효화 비용을 먼저 측정한 뒤 도입 여부를 결정한다;
  - 검토 범위: 거래/자산 목록 부분 갱신, 낙관적 업데이트, 서버 액션 캐시 전략과의 중복 여부;

#### P2 (AI 파싱/중복/모델 라우팅)

- [x] **AI-01** 자동 파싱 시 고정 거래(수입/지출) 중복 등록 방지 로직 추가
- [x] **AI-02** 입력 길이/이미지 유무 기준 모델 라우팅: 짧은 텍스트=Kimi 우선, 긴 텍스트/이미지=고성능 모델
- [x] **AI-03** 요청 크기별 timeout 차등화 + 사용자 안내 문구("분석 중/정리 중")
- [x] **AI-04** 긴 요청 처리 UX: 단계형 상태 메시지 + 취소/재시도 UX

#### P3 (데이터 모델/도메인 확장)

- [ ] **DM-01** 사용자 거래/자산 데이터 암호화 저장(필드 단위)
  - 우선 범위: **자산(name, balance) + 거래/수입/지출 데이터 전반(설명, 금액, 메모, 원문입력, 카테고리 등)**
- [x] **DM-02** 자산-거래 연동 모델 설계(계정잔액 변동 반영 규칙)
- [ ] **DM-03** 거래/수입 데이터 timestamp(초 단위) 저장 및 정렬 기준 고도화
- [ ] **DM-04** 고정거래 적용 내역을 "일괄 완료"가 아닌 "개별 적용 항목"으로 분리 표기
- [ ] **DM-05** 카드 부채 결제주기 모델링
  - 카드 사용일 지출, 청구기간, 결제일 실제 출금, 카드 부채 잔액 증감을 분리한다;
  - 카드사/결제일별로 다른 이용대금 반영 기간 규칙을 지원한다;
- [ ] **DM-06** 음수 지출/상계 거래 지원
  - `지출 -금액` 입력을 수입으로 전환하지 않고 지출 합계에서 차감한다;
  - 정산 회수, 일부 환급, 공동지출 회수를 일반 지출 차감 흐름으로도 기록 가능하게 정리한다;
- [ ] **DM-07** 고정거래 수동 적용을 도래 날짜 기준으로 제한
  - 현재 구현 확인: 자동 적용(`autoApplyRecurringTransactions`)은 `dayOfMonth <= 오늘`인 항목만 생성한다;
  - 현재 구현 확인: 수동 적용(`applyRecurringTransactions`)은 대상 월의 모든 고정거래를 한 번에 생성한다;
  - 목표: 자동/수동 정책을 맞추고, 사용자가 "이번 달 적용"을 눌렀을 때 아직 날짜가 오지 않은 항목은 제외하거나 별도 표시한다;
- [ ] **DM-08** 고정거래 수정 기능 추가
  - 현재 UI는 추가/삭제만 가능하고 기존 규칙 수정 진입점이 없다;
  - 범위: 설명, 금액, 날짜, 카테고리, 활성 상태 수정;

#### P4 (신규 기능/보안 운영)

- [ ] **AN-01** AI 소비습관 분석 기능(월간 패턴/카테고리 편중/절약 제안)
- [ ] **AN-02** 통계 화면 AI 요약/분석 추가
  - 월별 요약 카드, 카테고리 비중, 최근 추세를 바탕으로 "이번 달 소비 한줄 요약" 또는 상세 분석 카드 제공을 검토한다;
- [ ] **SEC-01** 악성 사용자 제한: 프롬프트 남용/토큰 소모/공격 패턴 rate limit + 단계별 차단
- [ ] **SEC-02** 입력 안전장치: OOD/프롬프트 인젝션/비정상 payload 방어 강화
- [ ] **OPS-01** 에러/성능 관측성에 `Sentry` 도입
  - 범위: 서버 액션 실패, LLM provider 오류, 복호화 실패, 주요 화면 성능 이슈 추적;
- [ ] **OPS-02** AI provider race 결과 메트릭 수집
  - provider별 승률, 평균 latency, 실패 유형, abort 비율을 기록해 현재 first-success-wins 전략의 비용/효율을 판단한다;
...
- 범위: TX-01~TX-11;
- 목표: 사용자가 즉시 체감하는 조작성/탐색성 문제를 해소한다;
- 산출물: 클릭 UX 개선, 월점프, 차트-필터 연동, 저장 후 리다이렉트 일관성, 입력/고정거래 편집 UX 개선;

#### Phase 2 (1~2주): 성능/로딩 아키텍처

- 범위: PF-01~PF-06;
- 목표: 페이지 전환 지연 체감 최소화와 상태 관리 구조 검토;
- 산출물: 섹션별 로딩, 지연 로딩, 프리패치/캐시 최적화, 로딩 카피 가이드, 상태 라이브러리 도입 판단 근거;

#### Phase 3 (1주): AI 파싱 안정성 + 라우팅

- 범위: AI-01~AI-04;
- 목표: 중복 등록 제거와 긴 요청 UX 안정화;
- 산출물: 모델 선택 정책, timeout 정책, 진행 상태 디자인;

#### Phase 4 (1~2주): 데이터 모델/보안 기반

- 범위: DM-01~DM-08, SEC-01~SEC-02;
- 목표: 저장 데이터 보안과 정합성 강화;
- 산출물: 암호화 저장 설계, 카드 부채 결제주기, 음수 지출 집계 규칙, 악성유저 제한, timestamp 정교화;
...
- 범위: OPS-01, OPS-02;
- 목표: 에러 추적과 AI provider 성능 판단 근거 확보;
- 산출물: `Sentry` 연동, provider별 latency/승률 대시보드 또는 로그 집계 기준;

### 기술 메모

- 암호화 저장: 앱 레벨 필드 암호화(AES-GCM) + 키는 KMS/시크릿 매니저 분리;
- 자산 연동: 거래 생성/수정/삭제 이벤트 시 account balance 증감 반영(옵션: 수동/자동);
- 카드 부채: `credit_card`는 사용 시 부채 증가, 결제 시 자산 감소 + 부채 감소로 분리;
...
5. 자산-거래 연동: **초기 등록 시 자산 선택 얼럿(선택형) 추가 + DB 저장**;
   - 단, 자산 선택 얼럿 노출 정책은 **설정에서 변경 가능**하게 제공한다;
6. 암호화 범위: **자산 + 거래/수입/지출 데이터까지 확대 암호화**;
7. 자산 선택 얼럿 트리거: **1회 기본 노출(1A)** + 설정에서 정책 변경 가능;
8. 고정거래 중복 판정: **금액+설명+카테고리+날짜근접(±1일) (2A)**;
9. 모델 라우팅 임계치: **짧은 텍스트 100자 이하 (3A-100자)**;
   - 단, **기존 Fireworks 3회 우선 룰이 최우선**이며 100자 룰은 그 다음에 적용한다;
10. 악성 유저 제한 단계: **soft 제한부터 시작 (4A)**;

### 구현 확인 메모

- **DM-02 자산-거래 연동**은 이미 구현 완료; 근거: [history](./history/2026-02-26-07-asset-transaction-linking.md);
- **텍스트 파싱 race 전략**은 이미 first-success-wins로 전환 완료; 근거: [history](./history/2026-03-17-17-text-parse-race-strategy.md);
- 고정거래는 **자동 적용은 도래일 기준**, **수동 적용은 월 일괄 적용**으로 현재 정책이 갈라져 있다;
- `zustand`, `react-query`는 아직 의존성에 없다;

### 다음 액션 제안

- 새 backlog 항목은 이 문서의 통합 backlog 섹션에만 추가한다;
- 세부 구현 전에는 해당 backlog 묶음을 기준으로 별도 `pipeline-state` 파일을 열고 승인형 Phase를 진행한다;
...
계획/백로그 변경도 `docs/implementation-plan.md`를 기준 문서로 유지하면서 함께 갱신한다.
...
| 2026-03-16 | 기능        | SEO 최적화 — robots.txt 및 Open Graph 메타데이터 추가                              | [history](./history/2026-03-16-12-seo-optimization.md)                                |
...
| 2026-03-17 | 문서        | 추후 작업 backlog와 구현 현황 정리                                                 | [history](./history/2026-03-17-22-future-work-backlog-alignment.md)                   |
| 2026-03-17 | 문서        | 구현 계획 단일 기준 문서화                                                         | [history](./history/2026-03-17-23-implementation-plan-unification.md)                 |
