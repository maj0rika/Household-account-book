...
| 항목        | 값                               |
| ----------- | -------------------------------- |
| 기본 폰트   | Pretendard Variable + Geist Sans |
| 모노 폰트   | Geist Mono                       |
...
```tsx
<Card>
	<CardContent className="p-3">{/* 내용 */}</CardContent>
</Card>
```
...
- 이메일/비밀번호 인증만 지원하며, 추가 소셜 로그인 버튼은 두지 않음
...
- 5개 항목: 홈, 분석, 입력, 자산, 설정
- `backdrop-blur-xl` 글래스모피즘
- 활성 탭: `text-primary font-medium`

### 데스크톱 (Sidebar)

- 6개 항목: 거래내역, 통계, 직접 입력, 자산/부채, 예산, 설정
- 상단: 앱 아이콘 + 이름
