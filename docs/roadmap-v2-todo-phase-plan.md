...
- [ ] **DM-05** 카드 부채 결제주기 모델링
  - 카드 사용일 지출, 청구기간, 결제일 실제 출금, 카드 부채 잔액 증감을 분리
  - 카드사/결제일별로 다른 이용대금 반영 기간 규칙 지원
- [ ] **DM-06** 음수 지출/상계 거래 지원
  - `지출 -금액` 입력을 수입으로 전환하지 않고 지출 합계에서 차감
  - 정산 회수, 일부 환급, 공동지출 회수를 일반 지출 차감 흐름으로도 기록 가능하게 정리
...
3. 모델 라우팅 임계치: **짧은 텍스트 100자 이하 (3A-100자)**
   - 단, **기존 Fireworks 3회 우선 룰이 최우선**이며 100자 룰은 그 다음에 적용
